...
2. 為了進一步以flash message顯示這兩種錯誤，會先做以下流程：
  - 啟用驗證過程能夠使用middleware的request物件，透過將passReqToCallback設定為true來進一步在驗證過程中調用connect-flash套件(建立flash-message)中的req.flash('error', message)
    ...
    ```
  - 透過passport.use的callback來呼叫done函式，並夾帶擁有message屬性的info物件執行，而done函式會間接執行req.flash('error', message)，而message內容則是info物件的message屬性
    ...
    - passport.authenticate內啟用failureFlash功能，允許done能夠執行req.flash('error', message)
