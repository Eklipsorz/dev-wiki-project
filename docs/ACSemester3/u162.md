...
由於單一餐廳頁面一次只處理一筆資料，所以方法比較簡單一點，只要讓 Handlebars 拿到 isFavorited 這個變數就好，不需要大費周章地重新整理一個餐廳大清單。因此在 restaurantController.getRestaurant 裡，我們的修改 controllers/restaurant-controller.js 方式會跟剛剛有點不同：
```
