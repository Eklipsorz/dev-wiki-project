...
1. handlebars 會依據block helper是否將渲染內容切分而將渲染檔案切割成parent context和child context，其中child context會是每個block helper能夠處理的內容，當這些helper執行時，往往只能夠辨識它們能理解的關鍵字(如this、this相關的變數、root、../等)，若遇到變數名稱，會直接視為空值或者不存在。
...
6. 細節：
  - 在這裡唯一會將渲染檔案內容切割成parent context和child context的block helper會是each helper，由於each本身要遍歷存有多個元素的物件，所以才特意切割，以保證遍歷到正確的元素。
  - 不會切割的helpers為if，所以{{#if variable}} 和 {{#each variable}}的variable皆屬於同一層
  ```
  {{#if variable}}
    {{#each variable}}
        context
