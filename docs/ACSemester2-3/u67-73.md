...
1. mongoose 模組物件會有的方法，主要會做兩件事情：
  ...
  - 檢查mongoose是否有建立滿足於對應schema之Document Instance的constructor，若有的話，從系統中獲取該constructor，若沒有的話，就建立滿足於schema之Document Instance 的 constructor
...
3. 其形式會是如下，於資料庫建立名為name(name變數值)的collection，並回傳符合schema的Schema 之 document constructor的形式來封裝：
```
mongoose.model(name, Schema)
```
