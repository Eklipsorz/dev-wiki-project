...
```
  ...
  let testModel = mongoose.model('applet', todoSchema)
  let srelloListModel = mongoose.model('applet', todoSchema)
  for (let index = 0; index < 10; index++) {
    srelloListModel.create({ name: `namet-${index}`, num: `${index}` })
  }

  console.log('isEqual: ', JSON.stringify(testModel) === JSON.stringify(srelloListModel))
```
