...
3. 其形式會是如下，會真實根據todoSchema來在資料庫建立名為names的collection，並以model的形式來封裝(編譯)該collection來回傳，其collection的資料會受限於todoSchema所規定的那樣，而之所以collection是names，而非name，請參考於 "mongoose.model 強制加s" 小節
```
mongoose.model('name', todoSchema)
```
4. 若已經封裝成對應collection的model，可以藉由名稱來得到對應的model，拿下面來當例子，第一行主要是定義並封裝model，而第二行則是藉由對應collection的名稱來找到對應的model回傳
...
5. mongoose.model 不允許修改已經定義好schema的model所擁有的schema，拿下面來當例子，假設name是一樣的，而todoSchema1和todoSchema2是相同Schema物件，那麼mongoose會因為沒修改而都回傳對應collection的model給開發者，可若todoSchema1和todoSchema2不是相同的Schema物件，即使兩者物件定義的schema內容是一樣，第二個mongoose.model會被mongoose判定為要修改已經定義好schema的model所擁有的schema而報錯。
```
