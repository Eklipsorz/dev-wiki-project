...
1. 使用{{ A }}來告訴系統請以Handlebars 語言來解析內容A，Handlerbars會試著辨識該內容A是否為變數、Helper，若是的話，會處理並回傳對應結果在{{ A }}的所在，若都不是的話，會直接用空值替代{{ A }}所在的地方
...
## Query String
1. 它作為URL的一部分，透過參數賦值來向特定資源A請求(索求)另一個資源B，換言之，會將特定資源A轉換成另一個資源B的形式，通常參數會在URL?之後，比如下面的parameter=value，parameter是參數名稱，而value則是對應參數的值
