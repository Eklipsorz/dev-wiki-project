...
3. 對model進行查找，在這裡由於要獲取對應collection的資料，所以沒設定任何filter，並進一步使用Query的lean來將資料庫的資料轉換成handlebars能夠接受的資料格式，最後回傳Query物件(內存格式轉換後的資料)

```
Todo.find() // 取出 Todo model 裡的所有資料
.lean()
```
4. 透過Query.then和.catch來進行非同步的事件事件綁定，當文件獲取並轉換成合適的格式之後，就便執行then，否則就執行catch
