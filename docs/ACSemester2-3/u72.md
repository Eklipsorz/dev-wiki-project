...
8. .catch則是單方面定義rejected所要有的事件處理，當狀態為rejected時，便會根據result來做後續處理
```
Promise.catch(onRejected);
```
9. .finally則是定義無論狀態為何都皆會執行的事件處理
```
Promise.finally(onFinally);
```

10. .then、.catch、.finally其擷取到的result值會取決於他們前一個.then、.catch所returen值或者當第一次從Promise本身透過resolve和reject而得到的值。
