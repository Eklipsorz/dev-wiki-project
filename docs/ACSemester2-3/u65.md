...
2. 由於該請求語言得基於Structured Data結構的關聯式資料庫來發送可辨識請求，Structured Data本身具有固定且可預知的特性，這可以進一步針對可能會有的操作結果來使請求拆分成幾個子句/子指令、目標為何、操作種類等等，而這讓請求具有結構性(structured query)，另外由於該請求是依據英語語系來建立，所以又稱之為structured english query
