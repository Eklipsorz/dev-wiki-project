...
## Graphical widget
1. Gaphical 為以圖形來呈現的，widget是指小產品，合在一起就是用圖形來呈現的小產品
2. 該詞有許多別名，主要是圖形化控制元件(Graphical Control Element) 或者 控制項 (Control)，較常見的別名是 控制項 (Control)
2. 在網頁開發或者軟體產品上，控制項是指圖形化介面上能與使用者進行互動的小元件，而這些小元件通常能給予使用者進行特定互動來進行特定資料的輸入、編輯、讀取，比如提供給人輸入的textarea。
