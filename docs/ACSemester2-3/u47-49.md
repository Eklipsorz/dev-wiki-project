...
```
5. 若 input 元件沒有value這屬性時，會以on來當(name屬性值對應的屬性)屬性值，拿上面的例子來說的話，若我們將代表Winter的輸入欄之value去掉並且點擊Winter，接著提交後會是以season=on來代表name為season的群組來傳遞資料

```
    <form>
        <input type="radio" name="season">Winter
        <input type="radio" name="season" value="spring">Spring
        <input type="radio" name="season2" value="summer">Summer
        <input type="radio" name="season2" value="autumn">Autumn
    </form>
...
5. 若 input 元件沒有value這屬性時，會以on來當(name屬性值對應的屬性)屬性值，拿上面的例子來說的話，若我們將代表Scales的輸入欄之value去掉並且點擊Scales，接著提交後會是以box=on來傳遞資料
