...
2. Middleware 對於請求X的處理，通常會是由一個針對於請求X的function集合(該集合在這被稱之為Middleware Stack)來進行處理，集合中的每一個Middleware function都能處理請求X，並且每個function皆能共享代表request和response的物件，也能對其進行修改，request物件代表著請求X本身，而response物件回應請求X的內容。
...
在瀏覽器讀取JavaScript的程式碼中，會先在這期間會定義該程式要處理哪種請求以及當對應請求接收時要執行哪些middleware function
根據請求在express.js的定義(如下)，
> 這裡的請求會是以 一個URL和對應URL的HTTP動詞 或者 一個URL 這兩種形式之一來構成

當瀏覽器讀取著用來 "定義對應請求X接收時要執行哪個middleware function" 的函式X 時，瀏覽器會檢查該函式所對應的URL和對應URL的HTTP動詞是什麼來定義請求X，而函式X的形式會是以下，app.use定義只包含URL的請求和定義callback為處理請求時所要用的middleware function，而app.method則是定義包含URL和HTTP動詞並且定義callback來處理該請求所要用的middleware function，另外method在這裡會是get、post等HTTP動詞。
