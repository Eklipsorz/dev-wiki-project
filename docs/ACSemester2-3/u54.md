...
同時檢測這個新增進來的Middleware function是否含有next參數、呼叫next()，若都有的話，會將下一個同個請求X下的函式X所含有callback以Middleware function 放入至 Middleware Stack ，並且是串接在前一個Middleware function之後，而每個擁有next參數的function都可以藉由next()去呼叫在它之後的 Middleware function。


![](https://res.cloudinary.com/dqfxgtyoi/image/upload/v1637065897/blog/middleware/addSecondMiddlewareFunction_j5nepi.png)


另外有添加next()參數和next()呼叫但卻沒有後續的callback可添加至Stack，就會執行執行

Middleware function若沒有next參數和next()呼叫的話，就不會讓後續的callback來加入Stack，


此時的Stack就會以目前收集到的function當作是該請求X下所要執行的函式，換言之，Stack就以現在的function來成形，
