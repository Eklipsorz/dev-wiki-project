...
2. Middleware 對於請求的回應，通常會是由多個 Middleware function 來組成一個function集合並協同完成請求，集合中的每個Middleware function皆可以共享於代表request和response的物件，也能進行修改。
...
1. 藉由express.js 所提供的 Middleware 來呼叫即可建立Middleware Function：
 - 應用程式種類： app.use(callback)，callback 會是該種類的Middleware，而app.use只是標明種類和URL(預設為'/')
 - 路由種類：router.use(callback)，callback 會是該種類的Middleware，而router只是標明種類和URL
 - 內建通用：express.static, express.json, express.urlencoded
 - 錯誤處理種類：app.use(err, req, res, next)
...
## 構成Middleware的方式
由回應同一個請求所經歷過的Middleware function來構成Middleware，通常構成方式大致上分為兩種，第一種為由程式碼的讀取先後順序來決定Middlware以及執行順序、第二種為由參數來預先指定，

### 程式碼的讀取先後順序來決定
這類型的 middleware function 皆由
