...
## body parser
1. 用來解析request object並轉換成


由於post是將資料存放於封包上的message body上，而post方法通常會使用URL encoding來編碼內容並放在body上，所以無法直接辨識其內容，而body parser正是解析URL encoding格式並轉換成原本內容至
...
而express在4.17.1以前並沒有一個好方法能夠方便取得post傳遞過來的資料，因此使用者必須使用body parser這套件來方便擷取，隨後4.17.1之後就內建了該套件來讓express能夠方便擷取post的資料。
