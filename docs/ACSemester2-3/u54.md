...
當瀏覽器讀取著加載Middleware function的函式X 或者 用來定義 "對應請求X接收時要執行哪個middleware function" 的函式X 時，瀏覽器會檢查該函式所對應的URL和對應URL的HTTP動詞是什麼來定義請求X，比如單純向根目錄'/'發送請求(如下圖的第一個)以及以get方式向'/'發送請求(如下圖的第二個)，在這裏系統會分別定義兩種請求：1. 根目錄'/' 2. 根目錄'/'和 get 動詞。
...
當能定義請求X時，就建立對應請求X的Middleware Stack，並且把該Middleware的Callback function放入至Stack中，同時檢測是否含有next參數、呼叫next()、是否會以res.send/res.render來回應請求
