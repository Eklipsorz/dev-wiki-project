...
4. 建立Stack的範例：當執行環境第一次讀取path1的app.use時，由於一開始沒Stack，所以會先建立Stack，緊接著就是將app.use中的path1和callback1轉換成Stack要存的類別-Layer，轉換後放推送至Stack中，接著處理完之後，再來對app.use(path2, callback2)進行處理，會將path2和callback2轉換成Layer類別推送至Stack中，它會放在Stack最後面，也就是第一個Layer之後，最後再對app.use(path3, callback3)進行處理，會將path3和callback3轉換成Layer類別推送至Stack中，它會被放在Stack的最後面，也就是第二個Layer之後。
