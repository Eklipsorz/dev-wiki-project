...
## 構成function集合的方式
由回應同一個請求所經歷過的Middleware function來構成function集合，而這個集合會以專門存放函式的Stack(以存放函式角度來用Stack稱呼，並非以next()來稱呼)來實現，換言之，這個Stack就會是專門存放Middleware function的集合


當接收某種請求時，便會從集合中找第一個Middleware function來執行，接著再由function自己呼叫next()來呼叫下一個function，最後由最後一個Middleware function來回傳response 物件給客戶端，但構成方式大致上分為兩種，第一種為由程式碼的讀取先後順序來決定Middlware以及執行順序、第二種為由參數來預先指定哪些函式放入集合以及順序。
