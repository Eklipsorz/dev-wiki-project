...
## 前端框架
1. 前端框架提出背景為 **在SPA中，客戶端會透過JS檔案來從後端獲取對應資訊或者狀態，根據資訊或者狀態的不同來更改客戶端畫面，其中當時實現上會是以指令式渲染來一步一步告知資料一變動的話程式如何渲染結果或者對應UI一變動的話資料如何變動，而這樣往往會使程式碼變得難以維護，甚至會打造成Spaghetti code**
