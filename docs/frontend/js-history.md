...
## 前端框架
1. 前端框架提出背景為 **由於AJAX本身會是非同步運算的語法，從AJAX獲取的資料、利用資料做相對應的DOM更新，也皆為以非同步來進行，而非同步運作下的，我們會因為無法確定請求什麼時候傳送、什麼時候收到請求的回應、什麼時候會利用獲取到的回應來進行渲染，所以因而無法確定最後結果是不是如我們預期的那樣**
2. 為了解決 **非同步帶來的不確定問題**，而提出前端框架來控管這些不確定，如Vue.js、React.js等前端框架來解決，由框架中的某一塊利用AJAX來更新網站的部分內容。
