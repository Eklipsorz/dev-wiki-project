...
2. 會是實際實現資料庫概念(包含建立並管理資料庫)的系統，具體來說會是
 - 定義該系統會有的資料：以Data Model來定義其結構、如何存取/操作、限制是如何等等(包含定義Schema來依據內容來確保每一次對資料上的操作都是符合Schema)
 - 提供一系列查詢語言API來幫助應用程式或者開發者透過API來產生對應指令讓系統解析並執行
 - 盡可能確保每一次操作和存取都能保證資料完整性(不破壞原資料內容)、資料一致性(而這也是關聯式資料庫的主要目標)
...
2. 關聯模型本身是基於數學(集合論、謂詞邏輯)形式來描述每個集合之間的關係本身，在這裡，關係本身就是以數學形式所描述的條件式，該關係會針對於多個集合中，而每個集合都存放著無窮無盡的資料，透過關係會從每個集合取出一個元素來滿足關係，關係最後的產物會是多個序列對，而每個序列對會是(x1,x2,x3,...,xn)，x1從X1集合取出，x2從X2集合取出，後面以此類推。
